...
Runs continuous evolution in small batches using Cron Triggers.

Architecture:
- Cron 1 (every 1 min): Generate 50 chaos trades → Store in D1
- Cron 2 (every 5 min): Analyze patterns if enough trades
- Cron 3 (every 10 min): Test strategies if patterns exist

This avoids timeouts by breaking work into micro-batches.
"""
...
class ChaosTradeGenerator:
    """Generate small batches of chaos trades"""

    def generate_batch(self, num_trades=50):
        ...
        for _i in range(num_trades):
            ...
            trade = {
                ...
                'buy_reason': self._generate_reason(),
                'buy_state': self._generate_state(),
                'sell_reason': self._generate_reason(),
                'sell_state': self._generate_state()
            }
            trades.append(trade)
    ...
    def _generate_state(self):
        return {
            'momentum_1tick': random.uniform(-0.05, 0.05),
            'vs_sma10': random.uniform(-0.1, 0.1),
            'volume_vs_avg': random.uniform(-0.5, 1.0),
            'volatility': random.uniform(0.01, 0.05)
        }


class PatternAnalyzer:
    """Analyze trades to find patterns"""
    ...
    def analyze(self, trades):
        ...
        winner_momentum_avg = sum(t['buy_state']['momentum_1tick'] for t in winners) / len(winners)
        loser_momentum_avg = sum(t['buy_state']['momentum_1tick'] for t in losers) / len(losers)

        pattern = {
            'id': f'pattern_{datetime.now().timestamp()}',
            'name': 'Momentum Pattern',
            'winner_momentum': winner_momentum_avg,
            'loser_momentum': loser_momentum_avg,
            'difference': winner_momentum_avg - loser_momentum_avg,
...
# Cloudflare Worker JavaScript equivalent below
...
# Save the worker
with open('cloudflare-workers/evolution-worker.js', 'w') as f:
    f.write(WORKER_JS)

print("✓ Created evolution-worker.js")
